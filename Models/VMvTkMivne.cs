using System;
using System.Collections.Generic;

namespace IHubWebApplication.Models;

public partial class VMvTkMivne
{
    public string? TeurTikModel { get; set; }

    public int KodTikModel { get; set; }

    public int? KodNeches { get; set; }

    public string? Neches { get; set; }

    public decimal? Mishkal { get; set; }

    public decimal MishkalEquity { get; set; }

    public int? KodSugModel { get; set; }

    public string? TeurSugModel { get; set; }

    public int? KodTatAfik { get; set; }

    public short? KodMachshir { get; set; }

    public string? Matbea { get; set; }

    public string? TeurTatAfik { get; set; }

    public string? TatAfikRama2 { get; set; }

    public string? Afik { get; set; }

    public string? MachshirMigbala { get; set; }

    public string? BasisHatzmadaMigbala { get; set; }

    public string MedinatChasifaMigbala { get; set; } = null!;

    public string KvutzatDerugMigbala { get; set; } = null!;

    public string NoDerugMigbala { get; set; } = null!;

    public string MenyautSikunMigbala { get; set; } = null!;

    public string HulChasifaMigbala { get; set; } = null!;

    public string IsKerenMigbala { get; set; } = null!;

    public string MedinaNischeretMigbala { get; set; } = null!;

    public decimal? Macham { get; set; }

    public decimal? TrumaLeMacham { get; set; }

    public decimal? DmeiNihul { get; set; }

    public decimal? MishkalHov { get; set; }

    public int? KodManpik { get; set; }

    public int? KodSugMishkal { get; set; }

    public int NiyarHarig { get; set; }

    public int TikHarig { get; set; }

    public string? Remark { get; set; }

    public string? ModelRemark { get; set; }

    public string? Isin { get; set; }

    public bool? IsHarig { get; set; }

    public string? HieTatAfikTeur5 { get; set; }

    public string? HieTatAfikTeur4 { get; set; }

    public string? HieTatAfikTeur3 { get; set; }

    public string? HieTatAfikTeur2 { get; set; }

    public string? HieTatAfikTeur1 { get; set; }

    public string? MachshirNb { get; set; }
}
